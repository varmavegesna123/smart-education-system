...
<head>
    ...
    <title>Sign In - Smart Education</title>
...
<body class="antialiased selection:bg-[#00C16A] selection:text-white min-h-screen flex overflow-x-hidden">
    ...
    <div class="hidden lg:flex w-1/2 relative overflow-hidden bg-[#050B14] flex-col justify-center p-16">
        ...
        <div class="relative z-10 max-w-lg xl:max-w-xl">
            ...
            <h1 class="text-3xl xl:text-4xl font-bold mb-4 leading-snug text-white tracking-tight">Empowering Students Through AI & Smart Learning</h1>
            <p class="text-gray-400 text-sm xl:text-base mb-12 font-light leading-relaxed max-w-md">Modern education platform designed to help students unlock their true potential through personalized analytics and guidance.</p>

            <!-- Mini Dashboard Preview / Analytics Cards -->
            <div class="relative">
                <div class="glass-card rounded-2xl p-6 border border-white/10 transform -rotate-2 hover:rotate-0 transition duration-500 w-4/5 relative z-10">
                    <div class="flex items-center justify-between mb-5">
                        <div class="flex items-center gap-3">
                            <div class="w-10 h-10 rounded-full bg-blue-500/20 flex items-center justify-center text-blue-400"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z"></path></svg></div>
                            <div>
                                <div class="text-sm font-semibold text-white">Performance Engine</div>
                                <div class="text-[11px] text-gray-500">Analyzing metrics</div>
                            </div>
                        </div>
                        <div class="px-2.5 py-1 bg-[#00C16A]/10 text-[#00C16A] text-[10px] font-bold rounded-md uppercase tracking-wider">Active</div>
                    </div>
                    <div class="space-y-3">
                        <div class="h-2 w-full bg-white/5 rounded-full overflow-hidden"><div class="h-full w-[85%] bg-[#00C16A] rounded-full"></div></div>
                        <div class="h-2 w-full bg-white/5 rounded-full overflow-hidden"><div class="h-full w-[60%] bg-blue-500 rounded-full"></div></div>
                        <div class="h-2 w-full bg-white/5 rounded-full overflow-hidden"><div class="h-full w-[75%] bg-purple-500 rounded-full"></div></div>
                    </div>
                </div>
                
                <!-- Floating Card Behind -->
                <div class="glass-card rounded-2xl p-5 border border-white/10 transform rotate-3 absolute top-12 left-1/4 w-3/4 -z-10 opacity-70">
                    <div class="flex items-center gap-3 mb-2">
                        <div class="w-8 h-8 rounded-full bg-emerald-500/20"></div>
                        <div class="h-2 w-20 bg-white/20 rounded"></div>
                    </div>
                    <div class="h-16 w-full bg-white/5 rounded mt-3"></div>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="w-full lg:w-1/2 flex items-center justify-center p-6 sm:p-12 relative">
        <div class="w-full max-w-[420px] relative z-10">
            <div class="text-center mb-8">
                ...
                <h2 class="text-2xl sm:text-3xl font-semibold mb-2 text-white tracking-tight">Welcome back</h2>
                <p class="text-sm text-gray-400 font-light">Sign in to your account to continue</p>
            </div>
            ...
            <div class="glass-card rounded-2xl p-6 sm:p-8">
                <form method="POST" action="{{ route('login') }}" class="space-y-5">
                    ...
                    <div x-data="{ show: false }">
                        <div class="flex justify-between items-center mb-1.5 ml-1">
                            ...
                            @if (Route::has('password.request'))
                                <a href="{{ route('password.request') }}" class="text-xs text-[#00C16A] hover:text-emerald-400 transition">Forgot password?</a>
                            @endif
                        </div>
                        <div class="relative">
                            <div class="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none">
                    ...
                    <div class="flex items-center pt-1">
                        <label for="remember_me" class="inline-flex items-center cursor-pointer group">
                            <input id="remember_me" type="checkbox" name="remember" class="w-4 h-4 rounded border-gray-600 bg-gray-800 text-[#00C16A] focus:ring-[#00C16A] focus:ring-offset-gray-900 transition">
                            <span class="ml-2 text-xs text-gray-400 group-hover:text-gray-300 transition">Remember me for 30 days</span>
                        </label>
                    </div>

                    <!-- Submit Button -->
                    <button type="submit" class="btn-primary w-full py-2.5 rounded-xl text-white font-medium text-sm shadow-lg flex justify-center items-center gap-2">
                        Sign In
                ...
                <div class="mt-6 text-center">
                    <p class="text-xs text-gray-500 font-light">
                        ...
                        <a href="{{ route('register') }}" class="text-white font-medium hover:text-[#00C16A] transition ml-1 border-b border-transparent hover:border-[#00C16A]">Create account</a>
                    </p>
                </div>
            </div>
            
            <div class="mt-8 text-center text-[10px] text-gray-600 font-light flex justify-center gap-4">
