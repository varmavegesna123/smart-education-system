...
<head>
    ...
    <title>Student Dashboard - Smart Education</title>
...
<body class="antialiased min-h-screen selection:bg-[#00D084] selection:text-white overflow-hidden" 
      x-data="{ 
          ...
          aiChat: {
              ...
              isTyping: false,
              messages: [
                  { role: 'ai', text: 'Hello! I am your personal AI learning assistant. How can I help you excel in your courses today?' }
              ],
              sendMessage(text = null) {
                  ...
                  setTimeout(() => {
                      this.isTyping = false;
                      this.messages.push({ role: 'ai', text: 'I am analyzing your request. This is a simulated response for demonstration purposes.' });
                  }, 1500);
              }
          }
      }">
    ...
    <div class="flex h-screen overflow-hidden bg-[#061224]">
        ...
        <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'" class="fixed inset-y-0 left-0 z-50 w-64 glass-panel flex flex-col transition-transform duration-300 lg:translate-x-0 lg:static lg:inset-0 lg:flex-shrink-0">
            ...
            <div class="flex-1 overflow-y-auto py-4">
                <nav class="space-y-1 px-3">
                    ...
                    <a href="{{ route('student.dashboard') }}" class="nav-link {{ request()->routeIs('student.dashboard') ? 'active' : '' }} flex items-center gap-3 px-4 py-2.5 rounded-lg text-sm font-medium text-gray-300 group">
                    ...
                    <a href="{{ route('student.courses') }}" class="nav-link {{ request()->routeIs('student.courses') ? 'active' : '' }} flex items-center gap-3 px-4 py-2.5 rounded-lg text-sm font-medium text-gray-300 group border-l-3 border-transparent">
                        <svg class="w-5 h-5 opacity-70 group-hover:opacity-100" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"></path></svg>
                        Courses
                    </a>
                    <a href="{{ route('student.reports') }}" class="nav-link {{ request()->routeIs('student.reports') ? 'active' : '' }} flex items-center gap-3 px-4 py-2.5 rounded-lg text-sm font-medium text-gray-300 group border-l-3 border-transparent">
                        <svg class="w-5 h-5 opacity-70 group-hover:opacity-100" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z"></path></svg>
                        Analytics
                    </a>
                    
                    <p class="px-4 text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2 mt-6">Learning Path</p>
                    <a href="#" @click.prevent="showToast('AI Insights updated')" class="nav-link flex items-center justify-between px-4 py-2.5 rounded-lg text-sm font-medium text-gray-300 group border-l-3 border-transparent">
                        ...
                        <span class="bg-[#00D084]/20 text-[#00D084] py-0.5 px-2 rounded-full text-[10px] font-bold">New</span>
                    </a>
                    <a href="#" @click.prevent="showToast('No pending remedial tasks')" class="nav-link flex items-center gap-3 px-4 py-2.5 rounded-lg text-sm font-medium text-gray-300 group border-l-3 border-transparent">
                        <svg class="w-5 h-5 opacity-70 group-hover:opacity-100" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4"></path></svg>
                        Remedial Tasks
                    </a>
                    <a href="{{ route('student.attendance') }}" class="nav-link {{ request()->routeIs('student.attendance') ? 'active' : '' }} flex items-center gap-3 px-4 py-2.5 rounded-lg text-sm font-medium text-gray-300 group border-l-3 border-transparent">
            ...
            <div class="p-4 border-t border-white/5">
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button type="submit" class="flex items-center gap-3 px-4 py-2.5 w-full rounded-lg text-sm font-medium text-gray-400 hover:text-white hover:bg-white/5 transition border-l-3 border-transparent">
        ...
        <div class="flex-1 flex flex-col h-full overflow-hidden relative">
            ...
            <div class="absolute top-0 right-0 w-[500px] h-[500px] bg-[#4F8CFF]/10 rounded-full blur-[120px] pointer-events-none z-0"></div>
            ...
            <header class="glass-panel h-16 flex items-center justify-between px-4 sm:px-6 lg:px-8 z-10">
                <div class="flex items-center gap-4">
                    ...
                    <div class="hidden sm:flex items-center relative">
                        <svg class="w-4 h-4 text-gray-500 absolute left-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path></svg>
                        <input type="text" placeholder="Search courses, insights..." class="bg-white/5 border border-white/10 rounded-full pl-9 pr-4 py-1.5 text-sm text-white placeholder-gray-500 focus:outline-none focus:border-[#00D084] focus:ring-1 focus:ring-[#00D084] w-64 transition">
                        <div class="absolute right-3 flex gap-1">
                            <kbd class="hidden lg:inline-block text-[10px] text-gray-500 font-mono bg-white/10 px-1.5 rounded">⌘</kbd>
                            <kbd class="hidden lg:inline-block text-[10px] text-gray-500 font-mono bg-white/10 px-1.5 rounded">K</kbd>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex items-center gap-4">
                    ...
                    <!-- Notifications -->
                    <button @click="showToast('No new notifications')" class="text-gray-400 hover:text-white relative transition">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9"></path></svg>
                        <span class="absolute top-0 right-0 block h-2 w-2 rounded-full bg-[#00D084] ring-2 ring-[#061224]"></span>
                    </button>

                    <!-- Profile Dropdown (Static for layout) -->
                    <div class="flex items-center gap-3 pl-4 border-l border-white/10 cursor-pointer" @click="showToast('Profile settings coming soon')">
                        <div class="text-right hidden sm:block">
                            ...
                            <p class="text-xs text-[#00D084] mt-1 font-semibold tracking-wider uppercase">Student</p>
                        </div>
                        <div class="h-9 w-9 rounded-full bg-gradient-to-tr from-[#4F8CFF] to-purple-500 flex items-center justify-center text-white font-bold text-sm shadow-md ring-2 ring-white/10">
            ...
            <main class="flex-1 overflow-y-auto overflow-x-hidden p-4 sm:p-6 lg:p-8 z-10 scroll-smooth">
                ...
                <footer class="mt-12 py-6 border-t border-white/5 text-center text-xs text-gray-500 font-light w-full">
                ...
                <div x-show="aiModalOpen" 
                    ...
                    <div class="px-6 py-6 pb-4 sm:p-6 sm:pb-4 border-b border-white/5">
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                <h3 class="text-base font-semibold leading-6 text-slate-100 tracking-tight" id="modal-title">AI Mentor</h3>
                    ...
                    <div class="px-6 py-3 border-t border-white/5 bg-slate-900/80 overflow-x-auto whitespace-nowrap scrollbar-hide">
                        <div class="flex gap-2">
                            <button @click="aiChat.sendMessage('Analyze my weak subjects')" class="text-[10px] px-3 py-1.5 bg-white/5 hover:bg-white/10 text-slate-300 rounded-full border border-white/5 transition tracking-wide">Analyze weak subjects</button>
                            <button @click="aiChat.sendMessage('Generate a study schedule')" class="text-[10px] px-3 py-1.5 bg-white/5 hover:bg-white/10 text-slate-300 rounded-full border border-white/5 transition tracking-wide">Generate study schedule</button>
                            <button @click="aiChat.sendMessage('Explain advanced calculus')" class="text-[10px] px-3 py-1.5 bg-white/5 hover:bg-white/10 text-slate-300 rounded-full border border-white/5 transition tracking-wide">Explain calculus</button>
                        </div>
                    </div>
                    ...
                    <div class="bg-[#0B1730] px-4 py-4 sm:px-6 border-t border-white/5 rounded-b-2xl">
                        <form @submit.prevent="aiChat.sendMessage()" class="relative flex items-center">
                            <input x-model="aiChat.input" type="text" placeholder="Message AI Mentor..." class="w-full bg-[#061224] border border-white/10 rounded-xl pl-4 pr-12 py-3 text-sm text-slate-200 placeholder-slate-500 focus:outline-none focus:border-purple-500 focus:ring-1 focus:ring-purple-500 transition shadow-inner font-light">
